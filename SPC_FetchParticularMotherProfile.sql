...
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchParticularMotherProfile' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchParticularMotherProfile 
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchParticularMotherProfile]
(	
	@HospitalId INT
...
BEGIN
		...
		(MD.[MotherSubjectId]  LIKE '%'+ @MothersRchSubHospID + '%'  OR MD.[HospitalNo] LIKE '%'+ @MothersRchSubHospID + '%'
		OR MD.[RCHID] LIKE '%'+ @MothersRchSubHospID + '%'  OR MD.[MotherFirstName] LIKE '%'+ @MothersRchSubHospID + '%'
		OR MD.[MotherLastName] LIKE '%'+ @MothersRchSubHospID + '%' OR MD.[MotherContactNo] LIKE '%'+ @MothersRchSubHospID + '%')			  
END