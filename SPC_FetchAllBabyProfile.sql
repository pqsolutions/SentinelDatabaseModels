...
GO
----Fetch Details of all baby subject 
IF EXISTS (SELECT 1 FROM sys.objects WHERE name='SPC_FetchAllBabyProfile' AND [type] = 'p')
BEGIN
	DROP PROCEDURE SPC_FetchAllBabyProfile--1,'',''
END
GO
CREATE PROCEDURE [dbo].[SPC_FetchAllBabyProfile]
(	
	@HospitalId INT
	,@FromDate VARCHAR(50)
	,@ToDate VARCHAR(50)
...
BEGIN
	DECLARE  @StartDate VARCHAR(50), @EndDate VARCHAR(50)
		...
		(CONVERT(DATE,BD.[DateOfRegistration],103) BETWEEN CONVERT(DATE,@StartDate,103) AND CONVERT(DATE,@EndDate,103))			  
END